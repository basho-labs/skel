...
start(Decomp, Reduce, NextPid) ->
  ...
  PidPools = dict:new(),
  NewReducer = spawn_link(sk_reduce_reducer, start, [DataReduceFun, NextPid]),
...
start_reducers(NPartitions, DataReduceFun, NextPid, PidPools) ->
  ...
  if
    TopPool < RequiredPool ->
      ...
      NewPids = [spawn_link(sk_reduce_reducer, start, [DataReduceFun, NextPoolPid]) || NextPoolPid <- TopPoolPids, _ <- [1,2]],
